...
export type ViewerWatchedMediaListQuery = { MediaListCollection?: Maybe<{ lists?: Maybe<Array<Maybe<(
      Pick<MediaListGroup, 'name' | 'status'>
      & { entries?: Maybe<Array<Maybe<WatchingMediaFragment>>> }
    )>>> }> };

export type MediaFragment = (
  Pick<Media, 'id' | 'format' | 'episodes' | 'bannerImage'>
  & { nextAiringEpisode?: Maybe<Pick<AiringSchedule, 'airingAt' | 'episode'>>, title?: Maybe<Pick<MediaTitle, 'english' | 'romaji' | 'native' | 'userPreferred'>>, coverImage?: Maybe<Pick<MediaCoverImage, 'extraLarge' | 'large' | 'color'>>, externalLinks?: Maybe<Array<Maybe<Pick<MediaExternalLink, 'id' | 'url' | 'site'>>>> }
);

export type ScheduleQueryVariables = Exact<{
  todaySeconds?: Maybe<Scalars['Int']>;
}>;


export type ScheduleQuery = { Page?: Maybe<{ airingSchedules?: Maybe<Array<Maybe<{ media?: Maybe<MediaFragment> }>>>, pageInfo?: Maybe<Pick<PageInfo, 'hasNextPage'>> }> };
